...
const Section = ({ className, unitId, visibleItems, visibleUpgrades }: SectionProps) => {
  ...
  return (
    <div className={ classNames(styles.wrapper, className) }>
      ...
      />
      <span>{ costText }</span>
      <b><button onClick={ handleReset }>Reset</button></b>

      { items.map((item, i) => (
        <div className={ styles.items } key={ i }>
          <span>{ item.name }</span>
          <b><button onClick={ () => buyItem(unitId, item) }>Buy</button></b>
          <span>
